...
Opal Crypto is the lowest-level cryptography package in the Swift stack. It exposes a strict, facade-first `OpalCrypto` namespace for keys, BCH and BIP340 signatures, bounded RSA blind signatures, NIP-44 v2 encrypted payloads, hashing, secure random bytes, encoding, derivation, and numeric helpers without leaking implementation details into downstream code.
