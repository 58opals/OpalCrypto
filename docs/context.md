...
Opal Crypto is the lowest-level secp256k1 cryptography package in the Swift stack. It exists so higher-level packages and apps can depend on one stable facade instead of coupling themselves to hashing, encoding, BCH or BIP340 signatures, NIP-44 encrypted payloads, secure randomness, mnemonic, derivation, or large-integer implementation details.
...
- In scope: facade-owned BCH cryptography, genuine BIP340 signing over typed 32-byte digests, NIP-44 v2 encrypted payloads, secp256k1 keys, bounded secure randomness, batch shared-secret computation, hashing, encoding, PBKDF2 key derivation, and numeric helpers.
...
- Prefer the facade surface such as `OpalCrypto.Signature`, `OpalCrypto.Nostr.NIP44`, `OpalCrypto.Key`, `OpalCrypto.Hashing`, `OpalCrypto.SecureRandom`, `OpalCrypto.Encoding`, `OpalCrypto.KeyDerivation`, `OpalCrypto.Numeric`, and `OpalCrypto.Secp256k1`.
