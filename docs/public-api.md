...
- `RSABSSA.MessageRandomizer`, `RSABSSA.BlindedMessage`, `RSABSSA.BlindSignature`, `RSABSSA.Signature`, `RSABSSA.VerificationKey`, `RSABSSA.BlindRequest.RecoveryState`
...
`RSABSSA` implements the randomized RFC 9474 flow for one strict profile: RSA-2048, exponent 65,537, SHA-384, MGF1-SHA384, a 48-byte PSS salt, and a 32-byte randomized message prefix. `SigningKey.generate()` creates an opaque, nonpersistent key that must be scoped to one higher-level protocol attempt and never reused for another protocol or parameter set. The private-alpha SPI can validate a newly created app-owned `SecKey` before its public identity is committed and requires an exact expected verification key when restoring an already committed capability. Its verification key uses the explicit RSA-PSS SubjectPublicKeyInfo form described by RFC 9578, and `keyIdentifier` is SHA-256 of that canonical DER document. A blind request exposes a versioned 401-byte `recoveryState` only for application-owned authenticated encryption; `restoreBlindRequest(message:using:from:)` revalidates its exact message, verification key, PSS encoding, and blinding factor before recreating the byte-identical blinded request.
