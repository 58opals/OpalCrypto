// Unsigned256BitIntegerModel~Arithmetic.swift

import Foundation

extension Unsigned256BitIntegerModel {
    @inlinable
    func add(_ other: Unsigned256BitIntegerModel) -> (sum: Unsigned256BitIntegerModel, carry: Bool) {
        ...
        return (Unsigned256BitIntegerModel(limbs: result), carry != 0)
    }

    @inlinable
    func subtract(_ other: Unsigned256BitIntegerModel) -> (difference: Unsigned256BitIntegerModel, borrow: Bool) {
        var result: InlineArray<4, UInt64> = .init(repeating: 0)
        var borrow: UInt64 = 0
        for index in 0..<4 {
        ...
        return (Unsigned256BitIntegerModel(limbs: result), borrow != 0)
    }

    @inlinable
    func multiplyFullWidth(by other: Unsigned256BitIntegerModel) -> Unsigned512BitIntegerModel {
        ...
        for leftIndex in 0..<4 {
            ...
            for rightIndex in 0..<4 {
                ...
                let (sum, newCarry) = Unsigned256BitIntegerModel.multiplyAdd(
                    low: low,
        ...
        return Unsigned512BitIntegerModel(limbs: result)
    }
    ...
    @inlinable
    func squareFullWidth() -> Unsigned512BitIntegerModel {
        var result: InlineArray<8, UInt64> = .init(repeating: 0)
