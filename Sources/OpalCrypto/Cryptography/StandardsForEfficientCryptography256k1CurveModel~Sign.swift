...
public extension StandardsForEfficientCryptography256k1CurveModel {
    static func sign(
        digest32: Data,
        privateKey32: Data,
        nonce: NonceGenerationPolicy.EllipticCurveDigitalSignatureAlgorithmModel = .rfc6979Sha256,
        ...
        switch nonce {
        case .rfc6979Sha256:
            var generator = try NonceGeneratorModel.RequestForComments6979Model(privateKey: privateKeyScalar, digest32: digest32)
            makeNextNonce = {
...
private func makeSystemRandomScalarForECDSA() throws -> ScalarModel {
    while true {
        ...
        guard status == errSecSuccess else {
            throw StandardsForEfficientCryptography256k1CurveModel.Error.randomGenerationFailed(status: status)
        }
