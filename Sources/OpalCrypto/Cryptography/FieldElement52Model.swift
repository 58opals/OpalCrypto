...
struct FieldElement52Model: Sendable, Equatable {
    ...
    @usableFromInline static let two = FieldElement52Model(fromUInt256: Unsigned256BitIntegerModel(limbs: [2, 0, 0, 0]))
    @usableFromInline static let three = FieldElement52Model(fromUInt256: Unsigned256BitIntegerModel(limbs: [3, 0, 0, 0]))
    @usableFromInline static let seven = FieldElement52Model(fromUInt256: Unsigned256BitIntegerModel(limbs: [7, 0, 0, 0]))
    @usableFromInline static let eight = FieldElement52Model(fromUInt256: Unsigned256BitIntegerModel(limbs: [8, 0, 0, 0]))
    
    @inlinable
    init(fromUInt256 value: Unsigned256BitIntegerModel) {
        var reduced = value
        if reduced.compare(to: StandardsForEfficientCryptography256k1CurveModel.ConstantModel.p) != .orderedAscending {
    ...
    @inlinable
    var asUInt256: Unsigned256BitIntegerModel {
    ...
    func negate() -> FieldElement52Model {
        ...
        let difference = StandardsForEfficientCryptography256k1CurveModel.ConstantModel.p.subtract(asUInt256).difference
    ...
    @usableFromInline
    static func makeLimbs(from value: Unsigned256BitIntegerModel) -> InlineArray<5, UInt64> {
    ...
    @usableFromInline
    static func makeUInt256(from limbs: InlineArray<5, UInt64>) -> Unsigned256BitIntegerModel {
        let limbMask: UInt64 = 0x000f_ffff_ffff_ffff
        ...
        return Unsigned256BitIntegerModel(limbs: [wordZero, wordOne, wordTwo, wordThree])
    }
}
