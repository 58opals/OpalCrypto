// RSABSSARequestForComments9474Validator.swift

import Foundation
@testable import OpalCrypto
import Testing

@Suite("RFC 9474 RSABSSA conformance")
struct RSABSSARequestForComments9474Validator {
    @Test("Match the randomized SHA-384 PSS client vector")
    func matchRandomizedSHA384PSSClientVector() throws {
        let modulus = try hexadecimal(Self.modulus)
        let verificationKey = try RSABSSAVerificationKeyModel(
            pkcs1Representation: RSABSSAKeyEncoding.makePKCS1PublicKey(
                modulus: modulus,
                publicExponent: 65_537
            ),
            expectedModulusBitCount: 4_096
        )
        let inverse = RSABSSAInteger(
            bigEndianRepresentation: try hexadecimal(Self.inverse)
        )
        let blindingFactor = try #require(
            inverse.inverseModulo(verificationKey.modulus)?
                .bigEndianRepresentation(
                    paddedTo: verificationKey.modulusByteCount
                )
        )
        let message = try hexadecimal(Self.message)
        let messageRandomizer = try hexadecimal(Self.messageRandomizer)
        let salt = try hexadecimal(Self.salt)

        var preparedMessage = messageRandomizer
        preparedMessage.append(message)
        #expect(preparedMessage == (try hexadecimal(Self.preparedMessage)))
        #expect(
            try RSABSSAModel.encodePSS(
                message: preparedMessage,
                salt: salt,
                modulusBitCount: verificationKey.modulusBitCount
            ) == hexadecimal(Self.encodedMessage)
        )

        let request = try RSABSSAModel.makeBlindRequest(
            message: message,
            verificationKey: verificationKey,
            messageRandomizer: messageRandomizer,
            salt: salt,
            blindingFactor: blindingFactor
        )
        #expect(request.preparedMessage == preparedMessage)
        #expect(request.blindInverse == (try hexadecimal(Self.inverse)))
        #expect(request.blindedMessage == (try hexadecimal(Self.blindedMessage)))

        let signature = try RSABSSAModel.finalize(
            blindSignature: try hexadecimal(Self.blindSignature),
            request: request,
            verificationKey: verificationKey
        )
        #expect(signature == (try hexadecimal(Self.signature)))
        #expect(
            RSABSSAModel.verify(
                signature: signature,
                preparedMessage: preparedMessage,
                verificationKey: verificationKey
            )
        )
    }

    private func hexadecimal(_ value: String) throws -> Data {
        try Data(
            hexadecimal: value
                .components(separatedBy: .whitespacesAndNewlines)
                .joined()
        )
    }

    private static let modulus = """
        
        48bdead5
        """

    private static let message = """
        8f3dc6fb8c4a02f4d6352edf0907822c1210a9b32f9bdda4c45a698c80023a
        a6b59f8cfec5fdbb36331372ebefedae7d
        """

    private static let messageRandomizer = """
        8417e699b219d583fb6216ae0c53ca0e9723442d02f1d1a34295527
        e7d929e8b
        """

    private static let preparedMessage = """
        8417e699b219d583fb6216ae0c53ca0e9723442d02f1d1a342955
        27e7d929e8b8f3dc6fb8c4a02f4d6352edf0907822c1210a9b32f9bdda4c45a698c8
        0023aa6b59f8cfec5fdbb36331372ebefedae7d
        """

    private static let salt = """
        051722b35f458781397c3a671a7d3bd3096503940e4c4f1aaa269d60300ce
        449555cd7340100df9d46944c5356825abf
        """

    private static let encodedMessage = """
        2be01c5669eb676cb3f0002eb636427d61568f3f0579da5b998279
        
        76f9fc073141e1e7bc
        """

    private static let inverse = """
        
        5fdec0991f
        """

    private static let blindedMessage = """
        aa3ee045138d874669685ffaef962c7694a9450aa9b4fd6465db9b
        
        039a7a92fe7efdd95d
        """

    private static let blindSignature = """
        3f4a79eacd4445fca628a310d41e12fcd813c4d43aa4ef2b81226953
        
        185969f70b1adf46
        """

    private static let signature = """
        
        cdfad5e0f2
        """
}
